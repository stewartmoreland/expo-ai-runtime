# Expo AI Runtime — reference cloud server

A tiny [Hono](https://hono.dev) backend implementing the wire protocol the
`@stewmore/expo-ai-cloud` adapter speaks. **Runs in mock mode by default** (no
API keys), so the `cloud-fallback` and `structured-output` examples work offline.
...
`OPENAI_BASE_URL` lets you point at any OpenAI-compatible server (OpenRouter,
Ollama, LM Studio, a local proxy, …).
