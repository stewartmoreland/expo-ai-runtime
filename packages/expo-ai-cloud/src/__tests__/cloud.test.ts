...
describe("CloudAdapter via ExpoAI", () => {
  it("generates text and tags privacy as third-party-cloud", async () => {
    const fetchImpl = (async () => jsonResponse({ text: "cloud reply", finishReason: "stop" })) as unknown as typeof fetch;
  ...
  it("parses SSE streaming deltas into chunks", async () => {
    ...
    for await (const chunk of ExpoAI.stream({ prompt: "hi", provider: "cloud" })) chunks.push(chunk);
  ...
  it("maps a 429 response to RATE_LIMITED", async () => {
    const fetchImpl = (async () => jsonResponse({ message: "slow down" }, 429)) as unknown as typeof fetch;
    configureCloud({ endpoint: "http://x", fetch: fetchImpl });
  ...
  it("parses CRLF-delimited SSE events", async () => {
    ...
    configureCloud({ endpoint: "http://x", fetch: (async () => sseResponse(events)) as unknown as typeof fetch });
  ...
  it("errors on a stream that ends with no content and no terminator", async () => {
    configureCloud({ endpoint: "http://x", fetch: (async () => sseResponse([])) as unknown as typeof fetch });
    await expect(streamText()).rejects.toBeTruthy();
  });

  it("maps an unknown upstream error code via HTTP status", async () => {
    const fetchImpl = (async () =>
      jsonResponse({ code: "rate_limit_exceeded", message: "slow" }, 429)) as unknown as typeof fetch;
    configureCloud({ endpoint: "http://x", fetch: fetchImpl });
    await expect(ExpoAI.generate({ prompt: "hi", provider: "cloud" })).rejects.toMatchObject({
      code: "RATE_LIMITED",
      retryable: true,
